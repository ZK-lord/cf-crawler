 ...
 * analyzer.c — 核心数据分析模块
 *
 * 功能：读取缓存的 CF API 数据，分析用户的比赛表现，
 *       统计提交情况，生成用于可视化的 JSON 数据树。
 *
 * 关键算法：
 *   1. 赛时/补题判定：根据提交时间与比赛结束时间比较
 *   2. AC 数量统计：按题目 rating 分桶，支持多时间窗口
 *   3. 180 天活跃度统计
 */
...
/*
 * 根据 rating 值返回对应的 CF 颜色（十六进制字符串）
 * CF 官方 10 级颜色体系，从 Newbie（灰）到 LGM（黑）
 */
...
/*
 * 建立比赛 ID -> {比赛名, 开始时间, 时长} 的哈希查找表
 * 用于快速获取每场比赛的起止时间，辅助赛时/补题判定
 */
 ...
 */
typedef struct {
    char index[4];        /* 题目编号：A, B, C, ... */
    int in_contest_ac;    /* 是否在赛内通过 */
    int post_ac;          /* 是否在赛后补题通过 */
    int rejected;         /* 错误提交次数 */
    int problem_rating;   /* 题目难度分 */
    int seen;             /* 在该场比赛的提交中是否出现过 */
} ProblemState;
...
/*
 * 处理某场比赛的所有提交记录，区分每道题的赛时/补题状态
 *
 * 算法核心：
 *   遍历 user.status 中 contestId 匹配的所有提交，
 *   如果某题有 creationTimeSeconds <= contestEnd 的 AC → 赛时通过
 *   如果只有 creationTimeSeconds > contestEnd 的 AC → 赛后补题
 *   没有任何 AC 且有过提交 → 赛时未通过
 *
 * 参数：
 *   status      — user.status 返回的完整提交数组
 *   contest_id  — 目标比赛 ID
 *   contest_end — 比赛结束时间戳
 *   probs       — 输出：分析结果数组
 *   max_probs   — 数组容量上限
...
static int process_contest_problems(cJSON *status, int contest_id,
    ...
    cJSON_ArrayForEach(sub, status) {
        ...
        if (strcmp(sverdict->valuestring, "OK") == 0) {
            ...
            probs[idx].rejected++;               /* 错误提交 */
        }
    }

    /* 按题目字母排序，保证输出顺序为 A, B, C, ... */
    qsort(probs, prob_count, sizeof(ProblemState), prob_cmp);
    return prob_count;
}

/*
 * 统计 AC 提交的题目难度分布（直方图数据）
 *
 * 遍历 user.status，过滤出 AC 提交，
 * 按题目 rating 分组计数，支持时间窗口过滤。
 *
 * 参数：
 *   window_sec — 时间窗口（秒），0 表示全部历史
 */
...
/*
 * 修正 API 返回的 rank 字段
 * CF API 有时会返回错误的 rank 数据（如 tourist 的 maxRank 返回的是用户名）
 * 因此总是根据 rating 值重新计算头衔
 */
 ...
 * 构建单个用户的完整分析数据树
 *
 * 读取 4 个缓存文件(user info / rating / status / contest list)，
 * 分析后生成包含以下信息的 cJSON 对象：
 *   - 基本信息（handle, rating, rank, avatar 等）
 *   - 统计数据（比赛次数、最高分、180天活跃度）
 *   - 比赛记录数组（含每道题的状态和补题情况）
 *   - AC 题目难度直方图（全部/一年/180天/一个月）
 *
 * 参数：
 *   handle        — Codeforces 用户名
 *   data_dir      — 缓存数据目录
 *   output_dir    — 输出目录（当前未使用）
 *   max_standings — 最大 standings 数量（当前未使用）
 * 返回：cJSON 对象指针，失败返回 NULL
 */
cJSON *build_user_summary(const char *handle, const char *data_dir,
    ...
    long long year_sec = 365LL * 86400;       /* 一年秒数 */
    long long halfyear_sec = 180LL * 86400;   /* 180天秒数 */
    long long month_sec = 30LL * 86400;       /* 一个月秒数 */
    ...
    cJSON_ArrayForEach(item, rating_history) {
        ...
        cJSON_AddNumberToObject(contest_obj, "startTime", (double)(start_time ? start_time : update_time));
